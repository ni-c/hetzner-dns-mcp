...
The MCP client decides which tools get called. Irreversible operations (`delete_zone`, `delete_rrset`, `import_zonefile`, `set_records`, `remove_records`, `change_primary_nameservers`, and the two `change_*_protection` tools when they remove protection) additionally require a server-issued confirmation token: the first call is refused and returns a random single-use token with a five-minute lifetime, and only a second call repeating the identical arguments executes. Because that token only ever appears in a previous tool result, content coming back from the API — record values, comments, zone files — cannot talk the model into producing one.
